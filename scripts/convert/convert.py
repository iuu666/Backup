...
def classify_and_extract_rule(line: str):
    ...
    if m:
        return m.group(1), 'perfect', None
    
    # 2. 可修正格式: 带路径或修饰符
    m = RE_FIXABLE.match(s)
    if m:
        return m.group(1), 'fixable_path', f'Extracted from path/modifier: {s[:80]}...'
    
    # 3. 可修正格式: hosts 风格
    m = RE_HOSTS.match(s)
    if m:
        return m.group(1), 'fixable_hosts', f'Extracted from hosts: {s[:80]}...'
    
    # 4. 可修正格式: 纯域名
    m = RE_PLAIN.match(s)
    if m and '.' in m.group(1):
        return m.group(1), 'fixable_plain', f'Extracted from plain domain: {s[:80]}...'
