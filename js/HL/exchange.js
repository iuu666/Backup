/**
 * 汇率监控脚本（基于 https://open.er-api.com/v6/latest/CNY）
 *
 * 功能汇总：
 * 1. 通过API获取人民币（CNY）基准的最新汇率数据
 * 2. 支持自定义汇率波动阈值（threshold，默认0.3%），超过阈值时发送系统通知提醒
 * 3. 支持开启/关闭通知功能（notify，默认开启）
 * 4. 支持参数传入自定义面板图标（icon）和图标颜色（color）
 * 5. 支持面板显示当前汇率信息及北京时间（Asia/Shanghai）数据更新时间和预计下次更新时间
 * 6. 汇率波动检测基于持久化缓存的上一次汇率数据
 * 7. 汇率显示支持美元、欧元、英镑、港币、日元、韩元、土耳其里拉等常用货币，格式化显示汇率及单位
 * 8. 全部时间均以北京时间（中文格式）显示，方便国内用户阅读
 * 9. 失败情况（请求失败、数据解析异常）面板会给出明确提示及图标标记
 */
